...
begin
  ...
  -- Use the run's start date (not current_date) so backfilled / synthetic runs
  -- attribute to the day they happened.
  ...
  segments as (
    -- ST_MakeValid is defensive: OSM polygons sometimes self-intersect, which
    -- crashes ST_Intersection with a TopologyException. Repairing on read is
    -- ~free for the small region count and lets bad data still produce results.
    select r.id as region_id,
           coalesce(
             st_length(
               st_collectionextract(
                 st_intersection(line.geom, st_makevalid(r.geom)),
                 2
               )::geography
             ),
             0
    ...
    from regions r, line
    where st_intersects(line.geom, st_makevalid(r.geom))
  )
  insert into region_scores as rs (region_id, user_id, date, distance_m, points)
  select s.region_id, v_user_id, v_date, s.dist_m, floor(s.dist_m / 10)::int
  from segments s
  where s.dist_m > 0
