...
type WinnerRow = { region_id: number; user_id: string; points: number };

const FALLBACK_CENTER: [number, number] = [-74.006, 40.7128];

export default function MapHome({ onRegionTap }: { onRegionTap: (regionId: number, regionName: string) => void }) {
  const { session } = useAuth();
  const [regions, setRegions] = useState<RegionRow[]>([]);
  const [winners, setWinners] = useState<Record<number, WinnerRow>>({});
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const parsed: RegionRow[] = (rs as Array<{ id: number; name: string; geojson: string }>).map((r) => ({
        id: r.id,
        name: r.name,
      ...
      const { data: ws } = await supabase
        ...
        .eq('date', dateStr);
      if (cancelled) return;
      const byRegion: Record<number, WinnerRow> = {};
      for (const w of (ws ?? []) as WinnerRow[]) byRegion[w.region_id] = w;
      setWinners(byRegion);
      setLoading(false);
    })();
    return () => {
  ...
  const fc: GeoJSON.FeatureCollection = {
    ...
    features: regions.map((r) => {
      ...
      const isMine = !!session && w?.user_id === session.user.id;
      return {
        type: 'Feature',
        id: r.id,
        properties: {
          regionId: r.id,
          name: r.name,
          fillColor: w ? userColorHex(w.user_id) : '#2a3b50',
          isMine,
        },
        geometry: r.geom,
      };
    ...
    <View style={styles.root}>
      <MapView style={styles.map} styleURL={Mapbox.StyleURL.Dark}>
        ...
        {regions.length > 0 && (
          ...
          >
            <FillLayer id="regions-fill" style={{ fillColor: ['get', 'fillColor'], fillOpacity: 0.45 }} />
            <LineLayer
              id="regions-line"
              style={{
      ...
      {!loading && regions.length === 0 && (
        <View style={styles.emptyBadge}>
          <Text style={styles.emptyText}>
            No regions loaded. Run `npx tsx scripts/fetch_regions.ts "&lt;city&gt;"` then `npx tsx scripts/load_regions.ts`.
          </Text>
        </View>
      )}
    </View>
...
const styles = StyleSheet.create({
  ...
  map: { flex: 1 },
  loadingBadge: { position: 'absolute', top: 60, alignSelf: 'center', flexDirection: 'row', backgroundColor: 'rgba(11,26,43,0.9)', padding: 12, borderRadius: 8, alignItems: 'center', gap: 8 },
  loadingText: { color: '#fff' },
  emptyBadge: { position: 'absolute', bottom: 100, left: 16, right: 16, backgroundColor: 'rgba(255,140,0,0.92)', padding: 14, borderRadius: 10 },
